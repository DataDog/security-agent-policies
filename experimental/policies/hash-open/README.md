...
This policy provides security rules designed to detect potential malware by computing hashes for every newly created file. The system generates events containing the hash values immediately upon file creation and subsequently raises security signals if these hashes match known malware signatures.
...
```bash
# Initialization
docker run -it -v ./policies:/policies -w /policies/hash-open \
